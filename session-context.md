...
**Last Updated**: 2026-07-28 (Correction session closing. Follow-up to the same-day PR #147-#150 close-out below — re-verified its "next session" list before accepting it rather than trusting it blindly.)
**Current Branch**: `main`
...
1. **DNS change for `api.bgclive.online`** — Railway's side is done (custom domain added); needs a DNS record change at whatever provider manages `bgclive.online`'s DNS: delete the stale A records for `api`, add CNAME `api` → `ei00i992.up.railway.app` and TXT `_railway-verify.api` → `railway-verify=e46e98e55ef27fd9ee2fbc569328e1508fb0b2ad5217ca60b13b8a7c3a353970`. Not urgent — nothing user-facing is broken today. Can take up to 72h to propagate once done.
