...
1. **Register the Resend webhook** — `docs/resend-webhook-setup.md` documents the exact steps; needs the backend actually deployed with PR #148's code first, then either the user or a future session needs to actually create the webhook endpoint in Resend's dashboard/API and set `RESEND_WEBHOOK_SECRET` in the real deployment env vars. Not done yet — the one loose end from the webhook work.
...
4. No other backlog items — repo is fully caught up, zero open issues/PRs as of this session's close.
