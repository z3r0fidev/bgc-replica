...
**Last Updated**: 2026-07-28 (Final closure session. Follow-up to the same-day correction session below — the user confirmed adding the `api.bgclive.online` DNS records at the external provider, independently re-verified end-to-end. This closes the last open item carried across three consecutive sessions: `390d50f` → PR #151 → PR #152. No work remains queued for this multi-session saga.)
**Current Branch**: `main`
**Session Status**: Closed. Four PRs merged (squash) in the original session: **PR #147** (`feat: unique usernames + 3 new Resend notification emails`, merge `1fe9120`), **PR #148** (`feat: add Resend bounce/complaint webhook`, merge `67495ca`), **PR #149** (`chore: track .env*.example template files, previously silently gitignored`, merge `024d90f`), **PR #150** (`fix(tests): exclude schemathesis contract tests from bare pytest by default`, merge `014db4e`). None of this session's work originated from filed GitHub issues — all four items came from direct user requests/scope decisions or findings raised and immediately actioned in the same session. **This correction session** then verified the resulting "next session" list rather than accepting it as-is, found the Resend webhook was in fact still unregistered, completed it plus a related Railway custom-domain fix, corrected all 4 context docs (`390d50f`→`f754b87`), discovered `main` is GitHub-protected (direct push rejected), and opened/merged **PR #151** (squash `0758813`) to land those 2 doc-correction commits the required way. HEAD after session: `0758813`. Zero open issues, zero open PRs, `main`-only branch state (confirmed via `git remote prune` — all stale feature-branch refs were already gone), all CI green at close except one confirmed-unrelated transient `npm ci` network flake on a non-required Playwright shard.

**Session shape**: started with the user asking to "set up emails for Resend." Investigation found Resend was already fully working (domain verified, 3 email types — verification/password-reset/warning — all Celery-queued). Asked the user what specifically was wanted; they chose: add new email types, verify deliverability end-to-end, add bounce/complaint handling. Discovering the backend's `notification_preferences` schema had 8 email toggles that had never actually triggered an email, and that there was no unique/parseable username field for `@mention` parsing, led to a bigger-than-planned username system — the user's explicit choice over two smaller alternatives (skip mentions, or fragile `display_name` matching). A single mistake recurred three times this session — a bash command missing its `DATABASE_URL=`/`REDIS_URL=` prefix silently falling through to `backend/.env`'s real production Supabase instead of the local test container — disclosed transparently each time (all three were schema-only changes with zero data impact, confirmed via read-only checks, and the user said "leave it, continue" each time since the changes were exactly what each PR would deploy anyway); after the third occurrence a mechanical fix (a wrapper script hardcoding the test DB URL) replaced "just be more careful." Two more findings surfaced near the end of the Resend work (uncommitted `.env*.example` templates, local bare-`pytest` flakiness); at the user's explicit request both were tackled immediately rather than deferred, using the user's requested process for the second one (senior agents, a `gemini-research` spawn, Plan mode with explicit approval before any code changes).

## Current State

### Latest Session — Username System + 3 New Resend Emails (PR #147), Resend Bounce/Complaint Webhook (PR #148), `.env*.example` Gitignore Fix (PR #149), Backend Test-Suite Flakiness Fix (PR #150)

**Starting point**: picked up from the 2026-07-27 close-out's zero-open-issues/zero-open-PRs/`main`-only baseline (`3a9c0b5`).

**Resend investigation (no PR, scoping conversation)**: user asked to "set up emails for Resend." Found Resend was already fully working end-to-end — domain verified (confirmed in the 2026-07-26 session), 3 email types (verification, password-reset, warning), all sent async via Celery. Asked what was actually wanted; user chose three concrete asks: (1) add new email types for messages/friend-requests/mentions, (2) verify deliverability end-to-end against the real Resend API, (3) add bounce/complaint webhook handling.

**Username system + 3 new emails (PR #147, squash `1fe9120`)**: building the message/friend-request/mention emails surfaced two pre-existing gaps: the backend already had a full `notification_preferences` schema (8 email toggles: `email_messages`, `email_friend_requests`, `email_mentions`, etc.) that had never actually triggered any email; and there was no unique username field (only a nullable, non-unique `display_name`) — no way to parse `@mentions` out of post content. Asked the user how to handle the mentions gap; they chose to add a real username field first (the larger option) over skipping mentions entirely or matching against the fragile, non-unique `display_name`.
- Added `users.username` (unique, indexed, nullable — mirrors how `email` is already nullable) via an Alembic migration with a Python-driven backfill (display_name → email local-part → user id, slugified and deduped on collision). Tested upgrade/downgrade against a disposable local Postgres with seeded edge cases: duplicate local-parts, special characters, NULL email.
- Made username required on registration (backend + frontend validation), changeable via a new `PATCH /api/auth/username`, with a new `UsernameCard` on the profile edit page.
- Added `send_new_message_email`/`send_friend_request_email`/`send_mention_email` to `email_service.py` plus their Celery tasks. Wired into `send_dm` (`socket_config.py`, only fires when the recipient is offline, checked via `presence_service`), `send_friend_request` (`social.py`), and new `@mention` parsing in `create_post` (`forums.py`). All three gated through a new shared `should_send_email()` helper (`app/services/notification_prefs.py`) respecting both the per-category preference booleans and `email_digest_frequency != "never"`.
- Verified all 6 email types (3 pre-existing + 3 new) end-to-end against the **real** Resend API — not mocked — all confirmed `delivered` via Resend's own event tracking, sent to the user's real inbox with their explicit approval.
- **Critical incident, disclosed transparently each time it happened**: hit the exact same mistake **three times** this session — a bash command missing its `DATABASE_URL=`/`REDIS_URL=` env var prefix silently fell through to `backend/.env`'s real production Supabase database instead of the local test container, running real Alembic migrations against production. All three times were schema-only (column/table additions), zero data impact since production's `users` table has 0 rows — confirmed via read-only checks each time, disclosed immediately, user said "leave it, continue as planned" each time (the schema changes were exactly what each PR would deploy anyway). After the third occurrence, stopped trusting "just be more careful" and built a mechanical fix: a wrapper script hardcoding the test DB URL for all subsequent alembic invocations, used for the rest of the session. This was saved as a feedback memory (`alembic_env_prefix_per_line_landmine.md`, updated 3x as it kept recurring) — flagged below as still needing a more permanent fix.

**Resend bounce/complaint webhook (PR #148, squash `67495ca`)**: `POST /api/webhooks/resend`, svix-signature-verified (new `svix` dependency in `backend/requirements.txt`). Fails closed with 401 if `RESEND_WEBHOOK_SECRET` is unset — deliberately reused the same 401 as a bad-signature response rather than a distinct 503, since schemathesis's contract-test fuzzing flags any 5xx as an automatic failure regardless of intent; this was actually caught by that exact contract test during verification. Persists `email.bounced`/`email.complained`/`suppression.added`/`suppression.removed` events to a new `email_events` table; other event types are acknowledged but not persisted. Added `docs/resend-webhook-setup.md` covering the one manual step the code can't do for itself.

**Webhook registration + a real production domain gap, both closed out same-session (after the initial session-close pass already ran — the user asked to verify the "next session" list before accepting it, which surfaced both of these)**: confirmed via Resend's API that no webhook was registered yet, then registered it directly against the real, working backend URL (`https://bgc-live-production.up.railway.app` — Railway's auto-generated domain, confirmed healthy via `/health`), set the returned `RESEND_WEBHOOK_SECRET` in Railway (triggered an auto-redeploy), and verified fully end-to-end with a real svix-signed test request against production: `200 {"received":true}`, then confirmed via a read-only production query that the row actually landed in `email_events`. Separately, discovered `api.bgclive.online` (the URL `.env.production.example` documents as "intended") has never actually worked — DNS resolves it to Vercel IPs with no matching deployment there, and Railway had no custom domain entry for it either. Confirmed this **isn't a live bug**: the real production frontend already works fine (`https://www.bgclive.online/api/search` returns real data), so whatever `NEXT_PUBLIC_API_URL` is actually set to in Vercel already points somewhere working. Added the custom domain on Railway's side (`railway domain api.bgclive.online` — safe/additive, doesn't touch the already-working Railway-assigned domain); the only remaining step is a DNS change at whatever provider manages `bgclive.online` (delete the stale A records for `api`, add a CNAME `api` → `ei00i992.up.railway.app` and a TXT `_railway-verify.api` → `railway-verify=e46e98e55ef27fd9ee2fbc569328e1508fb0b2ad5217ca60b13b8a7c3a353970`) — outside anything doable from this environment.

**Two more findings, discovered while wrapping up, tackled immediately at the user's request instead of deferred**:
- `backend/.env.production.example`, `frontend/.env.production.example`, and two `bgc-personals` `.env*.example` files were silently caught by the broad `.env*` gitignore pattern and had never been committed to the repo at all.
- Running the backend's full `pytest` (no `--ignore`) locally reliably failed 7 tests.

Both were saved as memories with the user's initial instruction to "investigate/address next session" — the user then immediately asked to tackle both now instead. Both were fully resolved this same session:

**`.gitignore`/env-example fix (PR #149, squash `024d90f`)**: added `!.env*.example` negation right after the `.env*` exclusion. Verified via `git check-ignore` both that all 4 template files are now trackable AND that real `.env`/`.env.local` files remain ignored. Manually reviewed all 4 files' contents first — placeholders/local-dev defaults only, no real secrets.

**Test-suite flakiness investigation + fix (PR #150, squash `014db4e`)** — user explicitly asked to use senior agents/skills, spawn `gemini-research` for deep understanding, and use Plan mode before implementing. Followed that process exactly:
- Spawned a `gemini-research` agent in parallel with a local empirical reproduction.
- Root cause confirmed three independent ways (local repro + `gemini-research`'s static code analysis + real GitHub Actions run history): `backend/tests/test_api_contract.py`'s schemathesis-fuzzed `TestClient` never gets `app.dependency_overrides[get_db]` applied (unlike `conftest.py`'s `async_client`), so its fuzzed requests make real, committed writes through the app's real `get_db`/`SessionLocal` — e.g. `POST /api/moderation/report` creates real `ContentReport` rows, breaking `test_moderation_api.py`'s exact-row-count assertions when both files share one pytest process. `test_main.py::test_health` failed too via a related connection-pool/event-loop interaction (it bypasses `Depends(get_db)` entirely, calling raw `SessionLocal()`/`get_redis()`).
- **Key discovery that reframed the fix**: checking `deploy-backend.yml`'s actual CI run history (`gh run view`) showed it already runs contract tests as a fully separate `pytest` step/process (`Run Contract Tests`, after a separate `--ignore`d `Run Tests` step) — two OS processes, each with its own fresh `test_engine`. There was **never any real CI risk** here; this was purely a "developer runs bare `pytest` locally" ergonomics gap.
- Entered Plan mode, wrote the plan to `/home/z3r0d3v/.claude/plans/smooth-discovering-wombat.md`, got explicit user approval before touching any code.
- Fix: one line, `addopts = --ignore=tests/test_api_contract.py` in `backend/pytest.ini` — the exact flag CI's own steps already use, now the local default too. Empirically verified pytest's explicit-target precedence over `--ignore` means `pytest tests/test_api_contract.py` still runs all 133 cases normally — zero CI workflow changes needed.
- Verified: bare `pytest` 3x on fresh local containers, 711 passed / 1 pre-existing xfail / 0 failures every time. Then verified live in the PR's own CI run that BOTH the "Run Tests" and "Run Contract Tests" steps in `deploy-backend.yml` passed independently — direct proof the fix doesn't disturb the one real place contract tests run separately.
- Found and deliberately did NOT chase a secondary issue: `GET /metrics`'s schemathesis case fails locally with an ad hoc/unpinned schemathesis+hypothesis pip install, but the exact same code passes cleanly in real CI (verified via actual run history) — documented as likely a local dependency-version artifact, not a live bug. Updated the `backend_full_suite_flaky_tests.md` memory to reflect the resolution.

**Final repo state this session**: 4 PRs merged (#147, #148, #149, #150); zero open issues, zero open PRs; only `main` branch anywhere; all CI green on `main`. All local scratch infrastructure (Docker containers, backend venvs in the ephemeral scratchpad, dev/prod servers, uvicorn processes) killed/removed/cleaned after each verification pass.

**Queued for next session (prominent — carry these forward until resolved)**:
1. ~~**DNS change for `api.bgclive.online`**~~ — **RESOLVED 2026-07-28 (final closure session)**. User confirmed the DNS records were added at the external provider; independently re-verified end-to-end via 4 checks: (1) `dig +short CNAME api.bgclive.online` → `ei00i992.up.railway.app.`; (2) `dig +short TXT _railway-verify.api.bgclive.online` → matches the expected `railway-verify=e46e98e55ef27fd9ee2fbc569328e1508fb0b2ad5217ca60b13b8a7c3a353970` exactly; (3) `railway domain` (from `backend/`) shows `api.bgclive.online` with sync status `ACTIVE`; (4) `curl https://api.bgclive.online/health` → `HTTP 200`, live. No longer queued.
2. **If `GET /metrics`'s schemathesis failure ever recurs in real CI** (it hasn't so far, confirmed via run history) — pin `schemathesis`/`hypothesis` versions in `backend/requirements.txt` and re-diagnose, per the note in `backend_full_suite_flaky_tests.md`.
3. **The alembic env-var-prefix landmine is still a live risk** — happened 3 times this session despite two prior rounds of "just be more careful" not sticking. A wrapper script was used for the rest of this session but that was ad hoc/scratchpad-only (not committed anywhere permanent) — worth considering a more permanent fix (e.g. a checked-in `scripts/test-alembic.sh` wrapper, or a Makefile target) so future sessions don't have to rediscover the same mitigation from scratch. Process/tooling note, not a code bug.
4. Resend webhook registration is **done** (see above) — no longer queued. No other backlog items — repo is fully caught up, zero open issues/PRs as of this session's close.

**Correction session (same 2026-07-28, follow-up conversation)**: before accepting the "Queued for next session" list above at face value, the user asked to verify it first rather than trust the prior close blindly. That verification is exactly what surfaced items 1-2 above (webhook still actually unregistered; the `api.bgclive.online` gap) — both were completed immediately at the user's explicit choice (via `AskUserQuestion`, "do both now" over deferring). The first close's docs (commit `390d50f`) had already been written and committed to `main` *before* that verification pass ran, so they still said "webhook pending" even after it was done. Corrected all 4 context docs in a follow-up commit (`f754b87`) removing the stale "pending" language. Attempting `git push origin main` with both commits was then rejected outright: `GH006: Protected branch update failed` — `main` requires the `quality-check` PR status check, and this is the first time a direct push to `main` was ever attempted in this project's session history (all prior work always went through `gh pr merge`, so this had never surfaced as a failure mode before). Saved as a new memory (`main_branch_protected_requires_pr.md`) so no future session tries a direct push again, even for trivial docs-only fixes. Opened **PR #151** (`docs/fix-stale-webhook-status`, containing exactly `390d50f`+`f754b87`), waited for its checks (`gh pr checks 151 --watch`): the required `quality-check` context passed on both instances it ran in (the "Docs PR" and "Frontend CI" workflows), and every other check passed except one non-required `playwright (4)` shard, which failed on a transient `npm ci` `ECONNRESET` network error during dependency install — confirmed unrelated to this docs-only PR's content, not a real regression. Per branch protection (`gh api .../branches/main/protection` confirms `quality-check` is the *only* required context) and this repo's own established precedent for merging past non-required failing checks (see the PR #122/PR #85 sessions above), squash-merged PR #151 (merge commit `0758813`, branch deleted) and synced local `main` to `origin/main` via `git fetch` + `git reset --hard origin/main` (working tree was already clean, nothing to stash). `git remote prune origin` cleaned up several already-stale local tracking refs for branches from earlier merged PRs (`feat/username-and-notification-emails`, `feat/resend-bounce-webhook`, `chore/track-env-example-files`, `fix/pytest-contract-test-isolation`, `fix/nextauth-rewrite-shadowing-144`, `security/next-auth-critical-cve-141`, `docs/fix-stale-webhook-status`) — all already deleted on `origin`, this was local-only bookkeeping. Final state re-confirmed: zero open issues, zero open PRs, only `main` anywhere (local and remote).

**Key technical/process lessons captured this session**:
- "Just be more careful" does not survive three repetitions of the same environment-variable-prefix mistake under time pressure — after the third occurrence, build a mechanical guard (a wrapper script, in this case) rather than relying on vigilance a fourth time.
- A broad `.env*` gitignore pattern silently swallows `.env*.example` template files too unless explicitly negated (`!.env*.example`) — check `git check-ignore` on template files periodically, since "should be committed" files can go untracked for a long time without anyone noticing.
- A local bare `pytest` failing where CI passes isn't automatically a false alarm or automatically a real bug — checking the CI workflow's actual run history (not just its YAML) settled definitively that contract tests already run in a fully separate process there, reframing what looked like a shared-state bug into a pure local-ergonomics fix.
- When schemathesis fuzzing can trigger any 5xx as an automatic contract-test failure, deliberately reusing an existing 4xx (401) for a "should never happen in production" misconfiguration case (missing webhook secret) can be the correct choice over a "more semantically correct" 503 — verified this was in fact caught by the contract test during this session's own verification pass.
- No destructive actions or unrecovered data-loss incidents this session — all three alembic-against-production incidents were schema-only changes against a table with 0 production rows, confirmed read-only each time before proceeding.
- **Never accept a prior session's "next session" queue at face value without spot-checking it first** — this correction session's entire reason for existing was that verification, not blind trust, and it found a genuinely stale item (the webhook) within minutes of checking.
- **`main` is GitHub-protected, requiring the `quality-check` status check** — a direct `git push origin main` fails outright (`GH006`) even for trivial docs-only commits. Every future session must go through `gh pr create` + `gh pr merge`, never a direct push, regardless of how small the change looks. Saved as `main_branch_protected_requires_pr.md`.
- A non-required CI check failing (here: one Playwright shard's `npm ci` hitting a transient network `ECONNRESET`) is not automatically a merge blocker — confirm via `gh api .../branches/main/protection` which contexts are actually *required* before treating any red X as blocking.

### Final Closure Session — `api.bgclive.online` DNS Confirmed Live (2026-07-28, third and final session of this saga)

**Context**: this is the third and final session closing out a single carried-forward item that spanned three consecutive sessions: the original PR #147-#150 session first identified the `api.bgclive.online` custom-domain gap and added the Railway side; a correction session (`390d50f` → PR #151) then verified that queue rather than trusting it blindly and found the webhook item still open, completing it; both sessions left exactly one item queued — the DNS record change itself, which requires access to the external DNS provider that isn't available from this environment. Nothing new was investigated or built this session; this was purely closing the loop on that queued item after the user made the external change.

**What happened**: the user confirmed they added the two required DNS records at the external provider — a CNAME `api` → `ei00i992.up.railway.app` and a TXT verification record `_railway-verify.api` → `railway-verify=e46e98e55ef27fd9ee2fbc569328e1508fb0b2ad5217ca60b13b8a7c3a353970`. Rather than take that at face value, independently verified end-to-end with 4 checks, all passing:
1. `dig +short CNAME api.bgclive.online` → `ei00i992.up.railway.app.` (matches)
...
3. `railway domain` (run from `backend/`) shows `api.bgclive.online` as a custom domain with sync status `ACTIVE`
4. `curl https://api.bgclive.online/health` → `HTTP 200` — live and working

**Result**: this closes the last open item from the entire Resend-webhook-and-DNS saga that spanned the PR #147-#150 session, the PR #151 correction session, and now this closure. There is no remaining "next session" item tied to this saga — repo state is zero open issues, zero open PRs, `main`-only, confirmed fresh this session.

**No new memory entry needed for the DNS confirmation itself**: it was a planned, expected external step with an outcome that matched exactly what was expected — not a landmine, not a surprising discovery, no lasting lesson to preserve. `main_branch_protected_requires_pr.md` (saved in the immediately-prior correction session) remains the relevant standing memory for how any future doc-only PR in this repo must be landed.

### Previous Session — Issue #141 (next-auth/next Critical CVEs, PR #143), Issue #144 (P0: NextAuth Routes Shadowed by Backend Rewrite, PR #145), Issue #142 (Gallery Add to Album, PR #146)

**Starting point**: picked up from the 2026-07-26 close-out's zero-open-issues/zero-open-PRs/`main`-only baseline (`b319b05`).

**Discovery sweep** (user: "Run the discovery sweep again for a fresh batch of gaps"): backend `ruff check .` (clean), frontend `npm run lint`/`tsc --noEmit` (clean, only pre-existing non-blocking warnings), `npm audit --omit=dev`, `pip-audit` against `backend/requirements.txt`, a TODO/FIXME grep (one hit, non-actionable), and a manual audit of query-param validation coverage across every `backend/app/api/*.py` router — this turned out to be a **false lead**: already fully covered via `validate_query_params`/`_assert_safe_string`/regex-constrained `Query()` patterns, no gap found (consistent with Issue #133 having already closed this class of bug in an earlier session). Two real findings surfaced and were filed as issues: **#141** (next-auth/next critical CVEs, P1) and **#142** (gallery "Add to Album" dead stub, P3). Also noted, explicitly **not** actioned: an `ecdsa` transitive CVE in `python-jose` — the backend only signs JWTs with HS256, the vulnerable ECDSA path is never exercised, and no upstream fix exists anyway for this pure-Python timing side-channel.

**PR #143 — next-auth/next CVE patch (Issue #141)**: `next-auth@5.0.0-beta.30` carried 2 critical + 1 high + 1 moderate live Auth.js CVEs (auth fail-open on config errors, homoglyph-email uniqueness bypass, uncaught exception on malformed Bearer headers, OAuth state/nonce/PKCE cookies not bound to their provider) — patched to `beta.32`. `next@16.1.0` separately carried a high-severity request-smuggling-in-rewrites CVE — bumped to `16.2.12` (non-major). `@auth/prisma-adapter` → `2.11.3`; ran `npm audit fix` (non-forced) for remaining safe transitive fixes. `npm audit --omit=dev`: 1 critical → 0. Verified via tsc, full Vitest suite (135 files/1276 tests), production build — all green. Pushed, opened PR #143.

**PR #143 CI failure and fix (same PR, user: "let's check the issues first and mitigate")**: CI failed at `npm ci`'s postinstall step across `frontend-check`, `quality-check`, and the Vercel deploy — `Cannot find module '.../@prisma/client/runtime/query_compiler_fast_bg.postgresql.wasm-base64.js'`. Root cause: `npm audit fix` had bumped the `prisma` CLI to `7.9.1` (fixing a real DoS-class CVE in `@prisma/config`/`@prisma/dev`) but left `@prisma/client` at `7.2.0` — a version-skew where the newer CLI's `prisma generate` expects a client layout the older `@prisma/client` package doesn't have. Fixed by bumping `@prisma/client`/`@prisma/adapter-pg` to `7.9.1` to match, regenerating the client, and re-verifying with a clean `npm ci` in a scratch dir (matching CI exactly) plus full tsc/test/build. Pushed, all CI green, merged (squash + delete branch). **Issue #141 closed.**

**Production incident discovered mid-verification (not a separate backlog item — found while smoke-testing #141's fix)**: checking that `next-auth`'s routes still worked after the version bump, `/api/auth/providers` returned 404 both locally (`next start`/`next dev` against a real backend) and, critically, confirmed live: `curl https://www.bgclive.online/api/auth/{providers,csrf,session,signin}` all returned genuine Next.js 404s. **Root cause**: `next.config.ts`'s generic `/api/:path*` → FastAPI-backend rewrite was returned as a plain array — Next.js's implicit **"afterFiles"** phase, checked *before* dynamic routes resolve. `src/app/api/auth/[...nextauth]/route.ts` is a dynamic catch-all, so every request to it matched the rewrite first and was proxied to the backend (no such routes there) instead of ever reaching NextAuth's own handler. **Impact**: `(auth)/login/page.tsx` calls `next-auth/react`'s `signIn()` for both Google OAuth and Passkey — both completely broken in production, for an unknown duration. Email/password login was unaffected (posts directly to the backend), presumably why this hadn't caused a loud outage report. Filed as **Issue #144 (P0)**.

**PR #145 — fix Issue #144**: first attempt moved the rewrite into Next's **`fallback`** phase (checked only after Next has tried and failed to resolve against its own routes, static and dynamic) instead of the plain-array/`afterFiles` form. Verified locally (clean build + `next start`/`next dev` against a real backend) — worked. Added `tests/e2e/nextauth-routing.spec.ts` as a regression guard: real, unmocked `request.get()` calls asserting on actual response status, unlike the existing `auth-google.spec.ts`/`auth-passkey.spec.ts` which only ever asserted a request was *made* (`page.waitForRequest`), never inspecting the response — exactly why they'd passed throughout this bug's entire lifetime. Verified the new spec fails pre-fix and passes post-fix. Pushed, opened PR #145.

**Second root cause, surfaced only against the live Vercel preview**: CI against the preview deployment still showed the exact same 404s — surprising, since local testing had passed. Investigated via the Vercel MCP tools (`get_deployment`, `get_deployment_build_logs`) and downloaded/inspected the actual Playwright trace.zip artifacts: confirmed the deployment was built from the correct commit, `[...nextauth]` compiled correctly into the build output, and the trace showed a real, authenticated 404 (Vercel's own SSO/deployment-protection bypass cookie present and working) — not a false negative. Root cause: `frontend/vercel.json` had a *duplicate*, unphased copy of the same `/api/:path*` rewrite. Despite general documentation claiming `next.config.ts`'s rewrites take precedence over `vercel.json`'s on Vercel, empirical evidence said otherwise here — `vercel.json`'s flat (afterFiles-equivalent) rewrite was still shadowing the dynamic NextAuth route on Vercel's actual routing layer regardless of the `fallback` phase in `next.config.ts`. Fixed by removing `vercel.json`'s rewrite entirely, making `next.config.ts` the sole source of truth. Pushed.

**Re-running CI surfaced a third, smaller wrinkle**: the new `nextauth-routing.spec.ts` tests now passed on every shard (proof the fix works), but this genuinely-fixed behavior broke two *pre-existing* `auth-passkey.spec.ts` assertions — they'd asserted the URL would land on `/login` or `/api/auth/error` after a passkey attempt, encoding the OLD BROKEN behavior (next-auth's client bailing out when its routes 404'd) rather than real NextAuth behavior. "passkey" was never even a real configured provider ID (`src/lib/auth.ts` only registers Google and Credentials) — with the routing fix, attempting to sign in with an unrecognized provider correctly reaches NextAuth's own `/api/auth/signin` page, standard next-auth behavior, not an error. Fixed both test assertions to accept `/api/auth/signin` as a valid outcome. Verified locally, pushed. Final CI run: all checks green, including all 4 Playwright shards against the live preview. Merged (squash + delete branch). **Issue #144 closed.** Verified live on production afterward: `curl https://www.bgclive.online/api/auth/providers` now returns 200 with real NextAuth provider JSON.

**PR #146 — gallery "Add to Album" (Issue #142)**: `gallery/page.tsx`'s bulk-select "Add to Album" button was fully wired into the UI but its handler just showed `toast.info("Add to album coming soon")` — the backend has had full album support since Spec 010 (`POST /api/gallery/albums/{album_id}/media`, etc.), this was purely a missing frontend dialog. Built `frontend/src/components/gallery/AddToAlbumDialog.tsx`: lists the user's albums (`GET /api/gallery/albums`), adds selected media to one on click, handles loading/empty/error states and the "media already in this album" case, and a "New Album" button that opens the existing `AlbumEditor` component and immediately adds the media to the newly-created album in the same flow. Wired into `gallery/page.tsx` replacing the stub. Added 9 new unit tests (`gallery-add-to-album-dialog.test.tsx`) and updated/replaced 1 stale test + added 1 new test in `app-gallery-page.test.tsx` (the old test asserted the "coming soon" toast, which no longer exists). Verified tsc/lint/full test suite (136 files/1286 tests)/production build all green.

**Manual verification hit a tooling/sandbox limitation, not an app bug**: spun up a full local stack (Docker Postgres/Redis, backend venv + uvicorn, seeded a test user + placeholder `GalleryMedia` rows via a scratch script, since real media upload needs live Supabase Storage credentials not loaded locally). The MCP browser tool can't resolve `localhost` to the host machine (only reachable via the host's raw LAN IP), and this app's enforced CSP `upgrade-insecure-requests` directive exempts `localhost` as a trustworthy context but not a plain IP — every JS chunk got silently upgraded to HTTPS and failed against the non-TLS dev server (`ERR_SSL_PROTOCOL_ERROR` on every chunk). Worked around by exercising the exact request/response contract `AddToAlbumDialog` uses directly via `curl` against the real local backend instead (create album → add media → re-add for idempotency — all matched exactly what the component sends/expects). The PR's live Vercel preview was also behind Deployment Protection SSO (correctly did not attempt to authenticate as the user there). Both limitations disclosed transparently in the PR description rather than claiming a full visual verification that didn't happen. Pushed, opened PR #146. All CI green (including all 4 Playwright E2E shards against the live preview, confirming no regressions). Merged (squash + delete branch). **Issue #142 closed.**

**Final repo state this session**: 3 PRs merged (#143, #145, #146); 3 issues opened and closed (#141, #144, #142); zero open issues, zero open PRs; only `main` branch anywhere; all CI green on `main`. All local scratch infrastructure (Docker containers `bgc-test-db`/`bgc-test-redis`, backend venvs in the scratchpad, dev/prod Next.js servers, backend uvicorn processes) killed/removed/cleaned up after each verification pass — nothing left running, `git status --short` clean.

**Key technical/process lessons captured this session**:
- `npm audit fix` can introduce version skew between tightly-coupled package families (here: `prisma` CLI vs `@prisma/client`/`@prisma/adapter-pg`) — after running it, check whether related packages that weren't individually flagged still match versions with what WAS bumped, and regenerate any codegen (`prisma generate`) before trusting a clean install.
- Next.js `rewrites()` returned as a plain array is the implicit **"afterFiles"** phase — checked *before* dynamic routes resolve. Any generic catch-all rewrite meant as a "proxy anything my own app doesn't handle" pattern needs the **`fallback`** phase instead, or it can silently shadow the app's own dynamic routes (this exact bug broke Google OAuth + Passkey login in production for an unknown duration).
- `vercel.json`'s `rewrites` can still take effect on Vercel's actual deployed routing layer even when `next.config.ts` has its own (differently-phased) rewrite for the same path — general documentation claiming framework-level config always wins was not reliable here; verify empirically via live preview deployment inspection rather than trusting the docs. When both configs exist for the same rule, prefer removing the duplicate rather than assuming precedence.
- E2E tests that only assert "a request was made" (`page.waitForRequest`) rather than inspecting the actual response can mask real, severe bugs indefinitely — `auth-google.spec.ts`/`auth-passkey.spec.ts` passed throughout this bug's entire lifetime for exactly this reason. New auth-adjacent E2E coverage should assert on response status/shape via `request.get()`, not just that a request fired.
- The MCP browser tool's network sandbox can't resolve `localhost` to the host machine, forcing raw-IP access — combined with an enforced CSP `upgrade-insecure-requests` directive (exempts `localhost` as trustworthy but not a plain IP), this breaks all local dev-server browser testing via that tool. Not fixable from the app side without weakening CSP; the practical workaround is verifying the exact API contract via `curl` against the same local backend, and/or a real Vercel preview if deployment protection allows unauthenticated access.
- No destructive actions or data-loss incidents this session.

### Previous Session — PR #128 (CSP Phase 2), PR #129 (Sentry Tracing), PR #130 (Spec 015 Phase 7 Verification)
