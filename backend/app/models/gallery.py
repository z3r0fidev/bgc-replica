...
from sqlalchemy import String, Boolean, DateTime, ForeignKey, Text, Integer, BigInteger, Index, CheckConstraint
...
class GalleryMedia(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("privacy IN ('PUBLIC', 'FRIENDS_ONLY', 'PRIVATE')", name="ck_gallery_media_privacy"),
...
class Album(Base):
    ...
    __table_args__ = (
        CheckConstraint("privacy IN ('PUBLIC', 'FRIENDS_ONLY', 'PRIVATE')", name="ck_albums_privacy"),
        Index("ix_albums_share_token", "share_token", postgresql_where="share_token IS NOT NULL"),
    )
...
class AlbumMedia(Base):
    ...
    media_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("gallery_media.id", ondelete="CASCADE"),
        primary_key=True
    )
    ...
    media: Mapped["GalleryMedia"] = relationship("GalleryMedia", back_populates="album_associations")

    __table_args__ = (
        Index("ix_album_media_position", "album_id", "position"),
    )
