...
class GroupMember(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    group_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("group_chats.id", ondelete="CASCADE"), index=True)
...
class GroupMessage(Base):
    ...
    sender_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
    content: Mapped[str] = mapped_column(Text)
    message_type: Mapped[str] = mapped_column(String(20), default="text")  # text, image, system
    reply_to_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("group_messages.id", ondelete="SET NULL"))
