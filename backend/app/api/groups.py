...
async def create_group(
    ...
    new_group = CommunityGroup(**group_in.model_dump(), owner_id=current_user.id)
    db.add(new_group)
    await db.commit()
    ...
    membership = GroupMembership(
        user_id=current_user.id, group_id=new_group.id, role="OWNER"
    )
    db.add(membership)
    await db.commit()
...
async def get_group_feed(
    group_id: uuid.UUID, db: Annotated[AsyncSession, Depends(get_db)]
):
