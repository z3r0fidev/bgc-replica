...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_gallery_media_user_created", "gallery_media", ["user_id", "created_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "cover_media_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("gallery_media.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column("privacy", sa.String(20), server_default="PUBLIC", nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "media_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("gallery_media.id", ondelete="CASCADE"),
            primary_key=True,
        ),
