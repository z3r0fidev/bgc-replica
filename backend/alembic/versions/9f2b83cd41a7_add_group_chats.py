...
def upgrade() -> None:
    ...
    op.create_table(
        'group_members',
        sa.Column('id', UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('group_id', UUID(as_uuid=True), sa.ForeignKey('group_chats.id', ondelete='CASCADE'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('id', UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('group_id', UUID(as_uuid=True), sa.ForeignKey('group_chats.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('sender_id', UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('content', sa.Text, nullable=False),
        sa.Column('message_type', sa.String(20), nullable=False, default='text'),
        sa.Column('reply_to_id', UUID(as_uuid=True), sa.ForeignKey('group_messages.id', ondelete='SET NULL'), nullable=True),
