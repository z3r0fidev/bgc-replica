...
def upgrade() -> None:
    ...
    op.add_column('sessions', sa.Column('created_at', sa.DateTime(), server_default=sa.func.now(), nullable=True))

    # Index for listing user sessions
    op.create_index('ix_sessions_user_id_last_active', 'sessions', ['user_id', 'last_active'])
